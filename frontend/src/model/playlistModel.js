export class PlaylistItem {
  ...
  isNewWithFile() {
    return this.isNew && this.file && !this.url_http && this.tipo !== 'texto_simples' && !this.mensagem;
  }

  canBeSent() {
    return this.isNew && this.file && this.arquivo && !this.url_http && this.tipo !== 'texto_simples' && !this.mensagem;
  }
    ...
    return this.monitores.flatMap(m => 
      (m.itens || []).filter(item => {
        if (item instanceof PlaylistItem) {
          if (item.url_http) return false;
          if (!item.isNew && (item.tipo === 'texto_simples' || item.mensagem)) return false;
          return item.isNewWithFile();
        }
        return false;
      })
    );
...
export function createPlaylistItem(data) {
