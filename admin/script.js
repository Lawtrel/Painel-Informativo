...
document.addEventListener("DOMContentLoaded", () => {
  ...
  addItemBtn.addEventListener("click", () => {
    ...
    if (
      !currentPlaylist.monitores ||
      !Array.isArray(currentPlaylist.monitores)
    ) {
    ...
    if (!monitor0) {
      // Se não existir monitor 0, cria um (simplificado)
    ...
    playlistJsonArea.value = JSON.stringify(currentPlaylist, null, 2);
    displayStatus(
      'Item de exemplo adicionado ao Monitor 0. Clique em "Salvar Playlist" para persistir.',
      "success"
    );
  });
  ...
  savePlaylistBtn.addEventListener("click", async () => {
    if (!currentPlaylist) {
      displayStatus(
        "Nenhuma playlist para salvar. Carregue ou modifique uma primeiro.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        displayStatus(
          result.message || "Playlist salva com sucesso!",
          "success"
        );
      } else {
        displayStatus(
          `Erro ao salvar playlist: ${result.message || "Erro desconhecido."}`,
          "error"
        );
      }
    } catch (error) {
